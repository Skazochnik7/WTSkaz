...
label giving_perfume(IsSmell=False): # perfume
    if not IsSmell:
        ...
        $hermi("Сэр... Это запах...")
        $hero("Какой запах?")
        $hermi("У вас башне пахнет... совсем как... это мне напоминает времена, когда я была маленькой.")
        if hermi.Items("perfume")._count==0:
            $hero("Неужели?")
            $hermi("Это же запах зубоврачебного кабинета!")
            $hero("Ох, бедное дитя, похоже, нелегко тебе приходилось.")
            $hermi("Да нет же, сэр, мне нравится этот запах и эти воспоминания!")
            $hero("Эмм... Девочка, я знал, что ты немножко мазохистка, но чтобы настолько...")
            $hermi("СЭР! Мои родители - стоматологи!")
