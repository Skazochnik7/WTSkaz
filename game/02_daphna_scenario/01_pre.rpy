...
#    skaz "Оставьте ваши вопросы, благодарности и пожелания на нашем {a=http://wtrus.ixbb.ru/viewtopic.php?id=9}ФОРУМЕ{/a}. \nТак вы простимулируете нас и продолжение появится быстрее. :)"
    ...
    $daphne.State(pos="door").Visibility("body+")("~55 00 1 pou// Добрый день, профессор Дамблдор.") 
#    $hermi.Visibility("body+")(".....")
    ...
    $daphne("~55 00 1 pou// Профессор Снейп сказал, что вы хотели меня видеть.") 
    $hero  ("Эм, да. Несомненно хотел... видеть.// Но, он же сказал зачем я хотел тебя видеть?") 
    $daphne("~55 00 1 pur", "Нет, сэр. Он не сказал.") 
    ...
    menu: 
        "Поговорить насчет учебы": 
            ...
            $daphne("Учеба не то, что меня волнует, сэр.// ~46 00 1 dis// Меня гораздо больше волнует, почему в Хогвартсе обучается грязнокровое мугродье.//" 
                "Вы же чистокровный волшебник, сэр?") 
            $hero  (g4, "Кто? Я?.. А, ну да. Вроде того.") 
            $daphne("36 00 1 opT",  "Да, вы по делу смущаетесь.// По древности рода с Гринграссами никто не сравнится, так что вы и должны чувствовать себя неловко.") 
            $hero  (m,"(Гринграссы? Что это еще за хрень?)") 
            $daphne("55 00 1 opT",  "Но как бы то ни было, сэр. Вы должны проследить, чтобы  мугродье не чувствовало себя здесь вольготно.", 
                "~55 n0 1 ang// А вы вместо этого приглашаете новых. И чистокровные девочки должны испытывать сложности.") 
            $hero  ("Чистокровные девочки?") 
            $daphne("55 00 1 ang",  "Да, сэр! Почему эту грязнокровку называют лучшей на курсе?//"
