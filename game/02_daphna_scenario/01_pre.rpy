...
label daphna_pre_01: #LV.1 (Whoring = 0 - 2)
    ...
    $snape.Visibility("head", transition=None)
    $hero("Послушай, Северус. Ты уже давно обещал прислать мне парочку слизеринских шлюх.")
    $snape("Гхм…")
    $hero("Что значит твое «гхм»?... Я точно знаю, у тебя есть шлюхи!// Ты сам мне недавно хвалился, что имеешь кого хочешь.")
    $snape("~02// Понимаешь, мой друг, все не так просто.//~05// Эти девки… Они уверены, что ты,… ну, директор, не в курсе наших маленьких шалостей.")
    $hero("И?")
    $snape("~03// И если окажется, что директору тоже не чуждо стремление к потас… к прекрасному, то это разрушит все их мировоззрение…")
    $hero("Послушай, дружище. Мне нет дела до мировоззрения шлюх, в них меня интересуют совсем другие… детали.") 
    $snape("~04// Да, но если они узнают о твоем интересе, они могут не захотеть больше этого делать.//~05// #(Надеюсь он проглотит эту хрень)//~07// #На самом деле я не собираюсь присылать ему шлюх -//~08//"
        " #все-таки джинн, обладающий крутой космической энергией и все такое.// #Вдруг им понравится больше?!")
    $hero("Ты думаешь, я проглочу эту хрень?")
    $snape("Хрень?// Мой друг! Я о тебе забочусь! Они реально могут перестать давать!")
    $hero("Давать кому, дружище?// Не тебе ли?// Я думаю, тебе просто нравится быть единственным петухом в курятнике.//"
        "Я тут не досыпаю ночей, думаю, как выдрессировать для тебя девчонку,...// а ты катаешься, как сыр в масле и не готов помочь мне с маленьким развлечением.")
    $snape("Но что тебя не устраивает! Я слышал у вас очень хорошо продвигается с мисс Грейнджер.")
    ...
    $snape("Но ты не можешь!…")
    $hero("Продолжай в том же духе и увидишь могу я или нет…")
    $hero("......")
    $snape("......// Ладно-ладно… Ты меня убедил. Я пришлю тебе шлюху.")
    $hero("Двух шлюх, Северус.")
    $snape("Я уверен, ЭТА тебе заменит двух сразу. ")
    $hero("Смотри, если это окажется какая-то уродка…")
    $snape("За кого ты меня принимаешь!// Чистая кровь в Мерлин знает каком поколении, была на обложке журнала, чирлидерша, слизеринка, наконец.// Девица - пальчики оближешь!")
    $hero("Ну, это ее забота... облизывать...// Так в чем подвох?")
    $snape("Подвох?")
    $hero("Ты слишком быстро согласился. Вот я и спрашиваю: в чем подвох? ")
    $snape("Никакого подвоха, мой друг! Ты сам убедишься. Мне нужно всего пару дней, чтобы все подготовить.")

    $event.Finalize()
    jump day_start


label daphna_pre_02: #LV.1 (Whoring = 0 - 2)
    ...
    menu:
        "\"Кто это?\"":
            ...
            $hero("#(Превосходно! Шлюха прибыла!// Надеюсь, не слишком страшна на вид, впрочем, после стольких дней воздержания я не слишком переборчив.)")
        "\"Да, входи...\"":
            pass

    $daphne.chibi.State("door", speed="go").Trans(d4, "go center", "blink")

    $screens.ShowD3("blktone")
