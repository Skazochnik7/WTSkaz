...
#    skaz "Оставьте ваши вопросы, благодарности и пожелания на нашем {a=http://wtrus.ixbb.ru/viewtopic.php?id=9}ФОРУМЕ{/a}. \nТак вы простимулируете нас и продолжение появится быстрее. :)"
    ...
    $daphne.State(pos="door").Visibility("body+")("~55 00z 1 smi// Добрый день, профессор Дамблдор.") 
#    $hermi.Visibility("body+")(".....")
    ...
    $daphne("~55 00z 1 def// Профессор Снейп сообщил, что вы хотели меня видеть.") 
    $hero  ("О да, девочка! Несомненно, несомненно хотел... видеть, да.// Но, он же сказал, зачем тебя прислал?") 
    $daphne("~55 00z 1 smi", "Нет, сэр. Он не сказал.") 
    ...
    menu: 
        "Поговорить насчет учебы": 
            ...
            $daphne("Учеба не то, что меня волнует, сэр.// ~46 n0z 1 def// Меня гораздо больше волнует, почему в Хогвартсе обучается грязнокровое мугродье.//" 
                "Вы же чистокровный волшебник, сэр?") 
            $hero  (g4, "Кто? Я?.. А, ну да. Вроде того.") 
            $daphne("~55 00z 1 pur",  "Хм. Вы по делу смущаетесь.// По древности рода с Гринграссами никто не сравнится, так что вы и должны чувствовать себя неловко.") 
            $hero  (m,"(Гринграссы? Что это еще за хрень?)") 
            $daphne("~46 00z 1 pur",  "Но как бы то ни было, сэр. Мама говорит, вы должны были проследить, чтобы  мугродье не чувствовало себя здесь вольготно.", 
                "А вы вместо этого каждый год принимаете новых.// ~46 01z 1 smi// И чистокровные девочки должны испытывать сложности.") 
            $hero  ("Чистокровные девочки?") 
            $daphne("~46 n0z 1 pur",  "Да, сэр! Почему эту грязнокровку называют лучшей на курсе?//"
                "Почему мугродью разрешено учиться рядом с истинными волшебниками?//" 
                "~64 n0z 1 pur// Я могу еще быть рядом со всякими Малфоями или Паркинсон.//" 
        ...
        "Поговорить насчет факультета": 
            ...
            $daphne("~46 n0z 1 def",  "Обстановка на факультете, сэр? Она отвратительна!// Грязнокровое мугродье заполонило Хогвартс.") 
            $hero  ("#(Что это за?..)") 
            $daphne("~55 n0z 1 ope",  "И вы делаете вид, что этого не замечаете. Хотя ваш отец...// Мама говорила мне, что он был истинным чистокровным волшебником и не побоялся прикончить троих маглов. За что его и упрятали в Азкабан.") 
            $hero  ("Правда, что ли? #(Ну и висельники в родстве с местным директором!)") 
            $daphne("~55 00z 1 dis",  "Не притворяйтесь, что вы этого не помните, сэр!// А если позабыли, это не делает вам чести!", 
                "~55 00z 1 pou// Гринграссы всегда стояли за чистоту крови и когда видишь волшебника, который предает наши идеалы...") 
    ...
    $daphne("~64 00z 1 smi// Приступим к чему, сэр?") 
    $hero  ("Ну, к этому самому... чпоки-чпоки, тити-мити, а?") 
    $daphne("~64 w0z 1 pur",  "\"Чпоки-чпоки\", сэр?") 
    ...
    $daphne("~64 w0z 1 pou// Я не понимаю, сэр.") 
    ...
    $daphne("~55 n0z 1 dis// На что это вы намекаете?!") 
    $hero  ("Я намекаю? Да я прямо говорю!...") 
    $daphne("~55 n0z 1 ang",  "Если вы смеете намекать на то, чем занимаются некоторые особы с преподавателями, то это омерзительно!") 
    $hero  ("#(Э-э, Снейп, это что, шутка такая?)") 
    $daphne("~55 n0z 1 ope",  "Я сегодня же пошлю сову родителям и сообщу о грязных предложениях, которые вы мне тут делаете.", 
    ...
    $daphne("~46 n0z 1 ope// Я все правильно поняла! Вам недолго осталось сидеть в этом кресле!") 
...
label daphne_pre_05: #Разборки со Снейпом после письма от родителей Дафны, само письмо лежит в 15_mail:daphne_pre_04
    ...
    $hero(m, "Ты меня круто подставил!")
...
# После того, как будет получена информация о выпивке Дафны ивент заканчивается. А так с интервалом в 2 дня при попытке поговорить возобновляется.
    ...
    if hermi.whoring<12:
        ...
        if end.IsEnding(const_ENDING_STRONG_GIRL):
            $hermi("~body_04.png// Все это напоминает методы ми-6, сэр, как показывают в сериалах. Вы случайно не работаете на спецслужбы?") 
            $hero  ("Очень смешно!") 
...
#    label daphne_pre_finish_menu:
    if event._finishCount==0: # Первый раз
        ...
        $daphne("Сэр, наша беседа идет в каком-то странном направлении! Если вы не прекратите, я немедленно уйду.") 
        ...
        $daphne("Сэр! Мне это не нужно, я – чистокровная волшебница в Мерлин знает каком поколении.// И я должна получать все просто так, безо всякого там покровительства!") 
        ...
        $daphne("О, сэр, это мое самое большое желание! Показать им их место. Показать, кто здесь действительно кто. Всем этим девкам...// Я уже столько лет в Хогвартсе и что же? Они до сих пор не разглядели с кем имеют дело!") 
