...
#    $hermi.Visibility("body+")(".....")
    ...
    menu: 
        "Поговорить насчет учебы": 
            ...
            $daphne("~55 00 1 pur",  "Хм. Вы по делу смущаетесь.// По древности рода с Гринграссами никто не сравнится, вы и должны чувствовать себя неловко.") 
            $hero  (m,"#(Гринграссы?)// #(Это слово очень подозрительно рифмуется... Наверняка означает какую-нибудь дрянь.)") 
            $daphne("~46 00 1 pur",  "Но как бы то ни было, сэр. Мама говорит, вы должны были проследить, чтобы  мугродье не чувствовало себя здесь вольготно.", 
                "А вы вместо этого каждый год принимаете новых.// ~46 01 1 smi// И чистокровные девочки должны испытывать сложности.") 
...
label daphne_pre_05: #Разборки со Снейпом после письма от родителей Дафны, само письмо лежит в 15_mail:daphne_pre_04
    ...
    $hero("Вот именно, она здесь и может видеть меня каждый день. И как это поможет ей забыть?")
    ...
    $snape("~05// Чтобы на следующий день весь Хогвартс обсуждал какой у директора размер?//"
        "~06// Эти шлюхи, когда их рот не занят полезным делом, только и делают, что болтают, и болтовня распространяется, быстрее, чем действие заклинания икоты.//"
    ...
    $snape("~03// Это так скучно. Давай я лучше пришлю тебе сову с выписками из ее студенческого личного дела с моими пометками.// Я их делал, когда всерьез планировал завалить ее.//"
...
# После того, как будет получена информация о выпивке Дафны ивент заканчивается. А так с интервалом в 2 дня при попытке поговорить возобновляется.
    ...
    if not event.IsExec( 1, "start1"): # Если не прошел день с начала самого первого старта, т.е. это и есть первый старт
        ...
        $hero  ("Причем здесь алфавит, великие пески! Первая девушка - совсем другая...")
        $hermi("~body_84.png// О, сэр! Я поняла. Вы хотите поговорить обо мне, ведь я первая по успеваемости. Это... это так трогательно...") 
        $hero("Да нет же, нет! Это - Дафна Гринграсс.") 
        $hermi("~body_185.png// Я не понимаю, сэр. Почему она первая?") 
    else:
            ...
            if hermi.liking>0:
                ...
                menu: 
                    "\"Будь по-твоему...\"":
                        ...
                        $hermi("~body_50.png// Ну, то, что ваша любовь первостатейная стерва я уже говорила.")
                        ...
                        $hermi("~body_103.png// Как будет угодно, сэр. Так вот, эта... девушка не имеет друзей кроме Коровы.")
                        ...
                        $hermi("~body_186.png// Простите, сэр, это я о Пэнси Паркинсон. Да и ту подругой можно назвать с натяжкой - недостаточно чистокровна для нее.//"
                            ...
                            "~body_111.png// Насчет взятия в рот я не осведомлена.")
                        $hero  ("Мисс Грейнджер! Вы становитесь несносны! Вы что, ревнуете?")
                        $hermi("~body_66.png// Ревную? К кому, сэр? //~body_186.png// К ЭТОЙ?// Какие глупости... // ~body_193.png// Я просто рассказывааю вам, как все обстоит на самом деле, ничего не приукрашивая.")
                        ...
                        $hermi("~body_100.png// Еще боится своей мамочки, а та боится, что о ней скажут люди - другие чистокровки.//"
                            ...
                            "~body_103.png// А! Еще ее сова Глатен - обжора еще та, только и ищет что бы слопать, не знаю интересно ли вам это...")
...
label daphne_pre_finish: #LV.1 (Whoring = 0 - 2)
    ...
    if IsNextRun():
        ...
        $daphne.ItemsCustomize(update={"combi:cheer_topbase", "skirt:cheer_long"})
