...
#    $hermi.Visibility("body+")(".....")
    ...
    menu: 
        "Поговорить насчет учебы": 
            ...
            $hero  (m,"#(Что это за... гринграссы?)// #(Как-то подозрительно рифмуется... Наверняка означает какую-нибудь дрянь.)") 
...
#    label daphne_pre_finish_menu:
    if event._finishCount==0: # Первый раз
        ...
        $daphne("~46 00 1 smi// Признаюсь, я изначально думала, что вы недостойны, чтобы учить меня - наследницу древнего рода.") 
        $hero  ("Хм...")
...
#    skaz "Оставьте ваши вопросы, благодарности и пожелания на нашем {a=http://wtrus.ixbb.ru/viewtopic.php?id=9}ФОРУМЕ{/a}. \nТак вы простимулируете нас и продолжение появится быстрее. :)"
    ...
    label daphne_pre_finish_menu:
    if item==None:
        $daphne("~55 01 1 pri// #Пообещал подарок и где он? Скряга...") Почему-то не срабатывает рот
        $daphne.liking-=5
    $screens.HideD3("bld1")
    $daphne.Visibility(transition=d3).chibi.Trans("goout door").Hide(d3)    
    $ renpy.play('sounds/door.mp3') #Sound of a door opening.
