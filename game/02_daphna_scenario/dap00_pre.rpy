...
#    label daphne_pre_finish_menu:
    if event._finishCount==0: # Первый раз
        ...
        $daphne("~55 w0 1 smi// О, как вы правы, сэр! Вы... вы сразу проникли в самую суть!"
        $hero  ("#(Еще не проник, дорогая, но и это не за горами.)")
        $daphne("Признаюсь, я изначально думала, что вы недостойны, чтобы учить меня - наследницу древнего рода.") 
        $hero  ("Хм...")
        $daphne("~55 w0 1 smi// Сэр, теперь я понимаю, что, кажется, была неправа.") 
        $hero  ("#(Ты даже не подозреваешь, девочка, насколько!)")
        $daphne("~46 n0 1 neuz// Профессор Дамблдор! Вот вы сразу заметили, что я особенная девушка! А это мугродье, они...") 
        $hero  ("#(Великие пески, только не это! Пора перевести ее мысли в более подходящее русло.)// Гхм! Мисс, жаль, что правила Хогвартса предписывают придерживаться единой формы верхней одежды.") 
        $daphne("~64 n0 1 pur// Да, сэр, это так. Но у меня и правда есть вкус, как у потомственной, чистокровной волшебницы. Такой вкус, что просто ух!//"
            "Будь у меня возможность, я бы им показала что такое настоящая стильная ведьма!") 
        $hero  ("Но у вас есть такая возможность, мисс.") 
        $daphne("~46 w0 1 smi// Есть?") 
        $hero  ("Вы же - чирлидер, насколько я знаю.// И форма чирлидера в Хогвартсе предполагает некоторую индивидуальность, и вы можете ее показать.// Я готов выступить в роли благодарного зрителя.")
        ...
        $daphne("~64 w0 1 dis// Спа-сибо, сэр. Но не кажется ли вам, что это несколько странно, если я буду приходить на занятия в этой форме. Она, несколько эм... вольная.") 
        ...
        $daphne("~26 n0 1 smo// О, сэр, это мое самое большое желание! Показать им их место. Показать, кто здесь действительно кто. Всем этим девкам...// ~26 n0 1 wo// Я уже столько лет в Хогвартсе и что же? Они до сих пор не разглядели с кем имеют дело!") 
        ...
        $daphne("~64 00 1 neu// О, сэр, все так быстро... я вообще-то не уверена,... я никогда до сих пор... Это нужно отменить!") 
        ...
        $daphne("~64 c0 1 pou// Н-нет, сэр, так нельзя. Но я не знаю что мне делать...// ~64 s0 1 ehh// Вы... вы правда поможете мне.") 
        $hero  ("Конечно, мисс Гринграсс, если вы будете делать то, что я скажу, вам не о чем волноваться.// Вы даже получите удовольствие.")
        $daphne("~55 s0 1 neu// Удовольствие, сэр? Не знаю, какое в этом может быть удовольствие.") 
        ...
        $daphne("~55 00 1 smi// О, вы о положении, о котором мы с вами говорили?") 
