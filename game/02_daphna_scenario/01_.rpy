...
#    $daphne.chibi.Hide(400, 0)
    $daphne.chibi.Pos("door", speed=20.0).Trans(["walk neardesk", "blink"])

    $daphne.SetViewMode(2)
    ...
    $hero  (m,              "О, девица вполне товарного вида. Сиськи, правда, маловаты. Интересно, есть ли заклинание, чтобы их подрастить?..",
                            ...
                            "Добрый день э-э… (великие пески, я не знаю, как к ней обращаться! Почему он не назвал ее имени?).")
    ...
    menu:
        "Поговорить насчет учебы":
            ...
            $daphne("55 00 3 opT",  "Ну, вы смущаетесь не зря.",
                                    "По древности рода с Гринграссами никто не сравнится, так я понимаю, что вы чувствуете себя неловко.",)
            ...
            $daphne("55 00 3 opT",  "Да, сэр! Почему это грязнокровка считается лучшей на курсе?",
                    ...
                    "55 00 3 opT",  "Я могу еще быть рядом со всякими Малфоями или …",
                                    "Семьи так себе, второй сорт. Но какие-никакие чистокровки, но эта девица!")
        "Поговорить насчет факультета":
            ...
            $hero  (                "#(Хм, какая экспрессия! Интересно трахается она так же вдохновенно?)",
            ...
            $hero  (                "О, великие пески пустыни!",
                                    "#(Она небыстро шевелит мозгами. Надеюсь, что бедрами она шевелит быстрее.)",
            ...
            $hero  (                "#(Не могу поверить... Чертов Снейп!!!)",
    ...
    $daphne.chibi.Mode("walk").Trans("walk door").Hide()
    ...
    $daphne.SetViewMode(3)
    $daphne("55 00 3 opT",  ".......")
    $daphne(                "Здравствуйте, профессор!")
    $hero  (m,              "Добрый день, мисс.") 


    $daphne.SetViewMode(1)
    $daphne("У меня к вам разговор!")
