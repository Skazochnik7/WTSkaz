label menu_reading_book:
    ...
    python:
        for e in this.List:
            if e._block==_block and e._status>=0:
...
# Проверяем, что освоены предыдущие ступени навыка:
                if event._block=="books_edu":
                    ...
                    if event==this.book_05_b and not this.book_05.IsDone():
...
# Music
    ...
    if raining:
        ">Вы читаете книгу под названием [event._caption], в тоже время слушая дождь, заливающий крышу вашей башни."
    else:
        ">Вы читаете книгу под названием [event._caption]..."
...
#===### SPEED READING FOR DUMMIES BONUS CHECK ###
    if s_reading_lvl == 1: #First book (book_08) on speed reading completed.
        ...
        if speed_dummies == 1: #Success.
            ">Используя изученные вами начальные методы скорочтения, вы более рационально используете время и продолжаете читать."
            call chap_finished_xx
            call chapter_check_book_xx #Checks if the chapter just finished was the last one.
            ">Осталось еще несколько глав."
    ...
    if s_reading_lvl == 2: #Second book(book_09) on speed reading completed.
        ...
        if speed_dummies == 1: #Success.chapter_check_book_xx
            ">Используя изученные вами продвинутые методы скорочтения, вы более рационально используете время и продолжаете читать."
            call chap_finished_xx
            call chapter_check_book_xx #Checks if the chapter just finished was the last one.
            ">Осталось еще несколько глав."
...
label chap_finished_xx:
    if event==this.book_05:
        $event.IncValue("status", 1)  #+=1
        $renpy.say("Глава [event._status]", 
        [
         "Галадриэль - нежная и добрая эльфийская принцесса.",
         "Отец Галадриэль - Король Метис и его друг детства Мофоселис.",
         "Король Метис объявляет о помолвке его дочери, принцессы Галадриэль и канцлера Мофоселиса.",
         "Галадриэль отказывается выйти замуж за человека, который в три раза старше ее и кого, до этой поры, она считала своим дядей.",
         "Король Метис не слушает \"глупые\" жалобы дочери. Галадриэль решает бежать.",
         "Галадриэль удается покинуть королевские покои ночью ...",
         "Король Метис в ярости из-за побега дочери. Он решает лично возглавить поисковую группу.",
         "Галадриэль едет на своем коне \"Белый голубь\" через лес. Принцесса наслаждается свободой... Вскоре она встречает красивого человеческого дворянина на лошади...",
         "Галадриэль едет вместе с дворянином. Они обмениваются бессмысленными шутками. Он очень веселит ее. Вдруг дворянин нападает на принцессу и вырубает ее!...",
         ...
         "Галадриэль безуспешно пытается вырваться. И тут она замечает, что ее окружают полдюжины мужчин, которые злобно ухмыляются.",
         ...
         "Вдруг все резко меняется. Галадриэль узнает, что она беременна. - Конец первой книги -"
        ][event._status-1]
        )


    if event==this.book_05_b:
        $event.IncValue("status", 1)  #+=1
        $renpy.say("Глава [event._status]",
        [   
         ...
         "Беременная Галадриэль предлагает составить компанию  паре грязных бездомных мужчин в обмен на еду. С ними она и ночует.",
         ...
         "Человек в маске кончает на лицо Галадриэль во второй раз за сегодня. Затем он решает снять свою маску, чтобы показать лицо.",
         "Галадриэль обнаруживает, что этот человек - ее отец, король Метис. Только тут и он понимает, что беременна шлюха, которую он трахал несколько часов - его дочь.",
        # Only now he realizes that the pregnant whore he fucked for hours is his daughter.
         ...
         "{size=-1}Галадриэль находится у алтаря. Она довольна и счастлива. Вдруг она замечает нечто, что наполняет ее сердце ужасом. На ее руке - шрам. Место укуса собаки. - Конец -{/size}"
        ][event._status-1]
        )

    if event==this.book_06:
        $event.IncValue("status", 1)  #+=1
        $renpy.say("Глава [event._status]", 
        [
        ...
        "Еще больше членов дворянской семьи северян померли.",
        ...
        "Персонаж за которого вы болели, влюбляется в милую девушку.",
        "Персонаж, за которого вы болели, жестоко убит. Его девушку насилуют и тоже убивают.",
...
label chapter_check_book_xx: #Checks if the chapter just finished was the last one.
    if (event.IsDone() and event!=this.book_07) or (event==this.book_07 and book_07_units == 20):#book_xx_units == 10:
        if fire_in_fireplace:
        ...
        if event==this.book_07:
            if complited_leena_already and complited_shea_already and complited_stevens_already and victoria >= 1 and shea >= 1 and leena >= 1: #Harem ending. The DAHR's ticket.
                ...
                if not complited_leena_already: #Finished with Leena for the first time.
                    ...
                    m "Хм... Может быть, я должен попробовать выбирать другие варианты в следующий раз...?"
...
# Можно было сделать через событие, но тогда получится более громоздко. Так что пока так:
        if event in [this.book_01, this.book_02, this.book_03, this.book_04]:
            $ concentration += 1
        if event in [this.book_05, this.book_05_b, this.book_06]:     
            $ imagination +=1
        if event in [this.book_08, this.book_09]:
            $ s_reading_lvl +=1
        if event in [this.book_12, this.book_13, this.book_14, this.book_15]:
            $ speedwriting += 1
