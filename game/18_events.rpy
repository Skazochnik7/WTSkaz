...
label event_08: # HERMONE SHOWS UP FOR THE FIRST TIME. IN USE.
    ...
    menu:
        ...
        "\"Кто это?\"":
            $ d_flag_01 = True
            $hermi("Это я, профессор...// Гермиона Грейнджер. Могу я войти?")
...
#    her "Но мы давно прошли это, не так ли?"
    ...
    $hermi("Я поняла бы, если бы это произошло несколько сотен лет назад...// Но мы живем не в темные века, не так ли?")
...
#    her "И что еще более важно - мотивация среди студентов неуклонно сокращается из-за этого!"
    $hermi("Это навредит системе распределения очков для факультетов.// И мало того!// Это навредит всей системе образования...// Из-за этого падает мотивация студентов!")
...
#    her "Доброе утро, профессор Дамблдор."
    $hermi.Visibility("body+", transition=d3)("~body_01.png// Доброе утро, профессор Дамблдор.")
    menu:
...
#            her "{size=-5}(Он назвал меня.... нет, показалось.){/size}"
            ...
            $hero(g4, "#Это было глупо... Джинни, джинни, следуй народной мудрости.// #\"Не доставай дрочило, пока она сама не захочет...\"// #\"...Или, хотя бы, заснёт.\"",
                    m, "*Кхм* Извините, что-то застряло в горле... Доброе утро, мисс Грейнджер.")
            $hermi("~body_202.png// #(Он назвал меня.... нет, показалось.)")
            
#    her "Профессор Дамблдор, Я здесь, чтобы поговорить с вами, как  президент \"ОЗПМ\" - Общества по Защите Мужских Прав..."
#    m "Защите прав?............."
